...
```console
docker-compose run --rm typescript-devtools help
```
...
FROM ghcr.io/coopnorge/engineering-docker-images/e0/devtools-typescript-v1:latest@sha256:7e54fe41351af1b7b4cdf75c2cb8251f80b89845b49179ae2003b200b3054369 AS typescript-devtools
```
...
- `ENTRYPOINT`: The Typescript file to use as an entrypoint to the application.

  This path must be a relative path to the root of your repo, or an absolute
  path.
...
- `OCI_REF_NAMES`: The space seperated OCI reference names that the
  ...
  e.g.
  `OCI_REF_NAMES=europe-docker.pkg.dev/sandbox-i^Cn-aucamp-e332/common-docker/typescript-devtools-prototype`
  will result in the image being published as
  `europe-docker.pkg.dev/sandbox-i^Cn-aucamp-e332/common-docker/typescript-devtools-prototype:latest`
  and
  `europe-docker.pkg.dev/sandbox-i^Cn-aucamp-e332/common-docker/typescript-devtools-prototype:gitc-${git_hash}`.
